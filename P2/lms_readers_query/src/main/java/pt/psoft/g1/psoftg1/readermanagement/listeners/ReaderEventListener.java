...
public class ReaderEventListener {
    ...
    public void validateReaderHandler(String readerId) {
        try {
            System.out.println(" [x] Received ValidateReader request for readerId: " + readerId);

            // Try to find reader by ID
            boolean readerExists = false;
            try {
                // Find reader using the readerId string directly
                readerExists = readerService.findByReaderNumber(readerId).isPresent();
            } catch (Exception e) {
                System.out.println(" [x] Error finding reader: " + e.getMessage());
            }

            // Send ReaderValidated event back
            readerEventPublisher.sendReaderValidated(readerId, readerExists);
            System.out.println(" [x] Sent ReaderValidated(readerId=" + readerId + ", exists=" + readerExists + ")");

        } catch (Exception ex) {
            System.out.println(" [x] Exception in validateReaderHandler: '" + ex.getMessage() + "'");
        }
    }
}

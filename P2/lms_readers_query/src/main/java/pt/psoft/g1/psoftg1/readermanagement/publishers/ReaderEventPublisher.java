...
public class ReaderEventPublisher {
    ...
    public void sendReaderValidated(String readerId, boolean exists) {
        try {
            // Create a simple validation response
            String jsonString = "{\"readerId\":\"" + readerId + "\",\"exists\":" + exists + "}";

            this.template.convertAndSend(direct.getName(), "reader.validated", jsonString);

            System.out.println(" [x] Sent ReaderValidated: " + jsonString);
        }
        catch( Exception ex ) {
            System.out.println(" [x] Exception sending reader validated: '" + ex.getMessage() + "'");
        }
    }
}

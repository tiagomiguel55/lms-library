...
@Disabled("Disabled until lending creation and search tests are updated for username input and Lombok getters")
class LendingServiceImplTest {
    ...
    void testCreateLending_ThrowsLendingForbiddenException_WhenUserHasOutstandingBooks() {
        ...
        when(reader.getReaderNumber()).thenReturn(readerNumber);
        when(readerRepository.findByUsername(username)).thenReturn(Optional.of(reader));
    ...
    void testCreateLending_ThrowsNotFoundException_WhenReaderNotFound() {
        ...
        when(readerRepository.findByUsername(username)).thenReturn(Optional.empty());

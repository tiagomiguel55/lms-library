...
public class BookRabbitmqController {
    ...
    /**
     * Listener para validação de livros quando um lending é solicitado.
     * Este é o ÚNICO listener relacionado com lending no BooksCmd.
     *
     * Fluxo:
     * 1. LendingsCmd solicita validação de um livro (envia LendingValidationRequest)
     * 2. BooksCmd verifica se o livro existe
     * 3. BooksCmd responde com LendingValidationResponse (via Outbox pattern)
     */
    @RabbitListener(queues = "lending.validation.request")
    public void handleBookValidationRequest(String message) {
        try {
            System.out.println(" [BOOKS] 📥 Received book validation request for lending");
            ...
            System.out.println(" [BOOKS] 🔍 Validating book with ISBN: " + request.getIsbn() +
                                 " | Lending: " + request.getLendingNumber() +
            ...
            try {
                // Verificar se o livro existe na BD do BooksCmd
                Book book = bookService.findByIsbn(request.getIsbn());
                response.setBookExists(true);
                response.setMessage("Book exists and is available");
                System.out.println(" [BOOKS] ✅ Book found: " + request.getIsbn() + " - " + book.getTitle());
            ...
            // Enviar resposta via Outbox pattern (não diretamente via RabbitTemplate)
            bookEventsPublisher.sendBookValidationResponse(response);

        } catch (Exception e) {
            System.err.println(" [BOOKS] ❌ Error processing book validation request: " + e.getMessage());
            e.printStackTrace();
        }
    }
